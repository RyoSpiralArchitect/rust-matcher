...
async fn record_queue_depth(pool: &PgPool) {
    let query = "SELECT \
    ...
    match pool.get().await {
        Ok(client) => match client.query_one(query, &[]).await {
            Ok(row) => {
                let pending: i64 = row.get("pending");
                let processing: i64 = row.get("processing");
                metrics::gauge!("llm_queue_pending_total").set(pending as f64);
                metrics::gauge!("llm_queue_processing_total").set(processing as f64);
            }
            Err(err) => {
                warn!(error = %err, "failed to fetch queue depth metrics");
            }
        },
        Err(err) => warn!(error = %err, "failed to get pool client for queue depth metrics"),
    }
}
