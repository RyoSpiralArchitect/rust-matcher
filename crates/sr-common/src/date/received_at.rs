...
mod tests {
    ...
    fn future_date(years_ahead: i32, month: u32, day: u32) -> NaiveDate {
        let year = Utc::now().year() + years_ahead;
        NaiveDate::from_ymd_opt(year, month, day).unwrap()
    }

    #[test]
    fn resolves_received_at_when_present() {
        let dt = future_date(1, 1, 15).and_hms_opt(9, 0, 0).unwrap();
        let resolved = resolve_received_at(Some(dt), None).unwrap();
        assert_eq!(resolved, future_date(1, 1, 15));
    }
    ...
    fn fallback_received_at_is_used_when_email_missing() {
        let fallback = future_date(0, 12, 1).and_hms_opt(0, 0, 0).unwrap();

        let resolved = resolve_received_at(None, Some(fallback)).unwrap();
        assert_eq!(resolved, future_date(0, 12, 1));
    }

    #[test]
    fn email_received_at_has_priority_over_fallback() {
        let email = future_date(1, 2, 10).and_hms_opt(15, 0, 0).unwrap();
        let fallback = future_date(0, 12, 1).and_hms_opt(0, 0, 0).unwrap();

        let resolved = resolve_received_at(Some(email), Some(fallback)).unwrap();
        assert_eq!(resolved, future_date(1, 2, 10));
    }
}
